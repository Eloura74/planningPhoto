...
function MyAccount() {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      CONFIRMED: { bg: "bg-green-100", text: "text-green-700", label: "✓ Confirmé", icon: "✓" },
      PENDING: { bg: "bg-yellow-100", text: "text-yellow-700", label: "⏳ En attente", icon: "⏳" },
      REQUESTED: { bg: "bg-blue-100", text: "text-blue-700", label: "📝 Demandé", icon: "📝" },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "var(--bg-primary)" }}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4" style={{ borderColor: "var(--gold-primary)" }}></div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 rounded-full flex items-center justify-center text-3xl" style={{ background: "var(--chrome-gradient)" }}>
              {user?.name?.charAt(0).toUpperCase() || "U"}
            </div>
            <div>
              <h1 className="text-3xl font-bold" style={{ color: "var(--gold-primary)" }}>
                Mon Compte
              </h1>
          ...
          <div className="flex gap-3">
            <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${user?.role === "ADMIN" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>
          ...
          {myBookings.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p style={{ color: "var(--text-muted)" }}>
                Vous n'avez pas encore de réservation. Consultez le calendrier pour réserver un créneau.
              </p>
            </div>
          ) : (
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                            {booking.slot_type === "GROUP" ? "Créneau Groupe" : "Créneau Solo"}
                          </p>
                        </div>
                      </div>
                      ...
                      {booking.status === "CONFIRMED" && (
                        <div className="mt-3 p-3 rounded-lg" style={{ backgroundColor: "rgba(16, 185, 129, 0.1)" }}>
                          <p className="text-sm font-semibold text-green-700">
                            ✓ Votre réservation est confirmée ! Rendez-vous le {formatDate(booking.slot_date)} à {booking.slot_start_time}
                          </p>
                        </div>
                      )}

                      {booking.status === "PENDING" && (
                        <div className="mt-3 p-3 rounded-lg" style={{ backgroundColor: "rgba(251, 191, 36, 0.1)" }}>
